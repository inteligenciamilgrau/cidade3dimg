...
for f in glob.glob(os.path.join(blocks_dir, "block_*.js")):
    ...
    if 'renderStandardSign(' not in content or 'renderStandardSign(monumentName, aiModel)' == content.split('renderStandardSign(')[-1].split(')')[0] + ')':
        content = content.replace("    };\n})();", f"        renderStandardSign('{monument}', '{ai}');\n    }};\n}})();".replace("}}", "}"))

    with open(f, 'w', encoding='utf-8') as file:
        file.write(content)

print('Fixed calls again')
